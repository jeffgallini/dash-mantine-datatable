...
[View on PyPI](https://pypi.org/project/dash-mantine-datatable/)<br>
...
<div><span><p><strong style="padding-bottom: 4px;">Jeff Gallini</strong></p></span><span style="margin-top: -0.15em; display: block;"><p><small>Maintainer</small></p></span><span style="margin-top: -0.15em; display: block;"><a href="mailto:gallinij@gmail.com" title="Email"><i class="bi bi-envelope-fill"></i></a> <a href="https://github.com/jeffgallini" title="GitHub"><i class="bi bi-github"></i></a></span></div>

#### Community

[Contributing guide](contributing.qmd)<br>
...
![Basic formatting example](assets/examples/hero-basic-formatting.png)
...
Build the Great Docs site locally:

```bash
python -m pip install -e ".[docs]"
quarto --version  # requires Quarto 1.5.0 or newer
python scripts/build_docs.py
```

This regenerates the recipe pages from `usage.py`, builds the Great Docs site,
and copies the rendered output to `site/`. Great Docs also generates
`llms.txt`, `llms-full.txt`, Markdown page copies, and the API reference from
the public Python exports.

On Windows, `great-docs` installs JupyterLab assets with very long filenames.
If `python -m pip install -e ".[docs]"` fails under a deeply nested `venv`
path, either enable Windows Long Paths or create the virtual environment in a
shorter directory such as `C:\venvs\dmdt`.

```powershell
py -m venv C:\venvs\dmdt
C:\venvs\dmdt\Scripts\Activate.ps1
Set-Location C:\Users\galli\Documents\GitHub\Repositories\dash-mantine-datatable
